...
KEXT_STATIC bool DirectoryContainsPath(const char* directoryPath, const char* path);
...
VirtualizationRootHandle ActiveProvider_FindForPath(const char* _Nonnull path)
{
     ...
     RWLock_AcquireShared(s_virtualizationRootsLock);
    {
        for (uint32_t i = 0; i < s_maxVirtualizationRoots; ++i)
        {
            ...
            if (root.inUse && root.providerUserClient != nullptr && DirectoryContainsPath(root.path, path))
            {
                matchingHandle = i;
                break;
            }

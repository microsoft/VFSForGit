...
namespace GVFS.UnitTests.Maintenance
{
    ...
    public class PackfileMaintenanceStepTests
    {
        ...
        [TestCase]
        public void PackfileMaintenanceFastPathVerifiesOnlyNamedPackWhenErrorNamesIt()
        {
            this.TestSetup(DateTime.UtcNow);
            this.SetupVerifyFailsOnceWithNamedPackError("pack-2.pack");

            // Only pack-2 should be verified via the fast path - no other pack's verify-pack result is
            // even registered, so the test would fail with an unexpected-command error if the fallback
            // full scan ran instead.
            this.gitProcess.SetExpectedCommandResult(
                this.VerifyPackCommand("pack-2.idx"),
                () => new GitProcess.Result(string.Empty, "error: could not load pack\n", GitProcess.Result.GenericFailureCode));
            ...
            List<string> commands = this.gitProcess.CommandsRun;
            commands.Count(c => c.StartsWith("verify-pack ")).ShouldEqual(1);
            commands.Where(c => c.StartsWith("verify-pack ")).Single().ShouldEqual(this.VerifyPackCommand("pack-2.idx"));
        ...
        [TestCase]
        public void PackfileMaintenanceFallsBackToFullScanWhenNamedPackIsHealthy()
        {
            this.TestSetup(DateTime.UtcNow);

            // The verify failure text names pack-1, but pack-1 turns out to verify successfully; the
            // real corrupt pack (pack-2) is only found once the code falls back to the full scan.
            this.SetupVerifyFailsOnceWithNamedPackError("pack-1.pack");
            ...
            // 1 fast-path verify-pack (pack-1, healthy) + 3 full-scan verify-pack (pack-1/2/3).
            commands.Count(c => c.StartsWith("verify-pack ")).ShouldEqual(4);
        ...
        /// <summary>
        /// Makes the multi-pack-index verify fail the first time with an error that names
        /// <paramref name="packFileName"/> directly (as real git verify failures do - e.g. "failed to
        /// load pack entry for oid[0] = ..." followed by "packfile pack-2.pack does not match index"),
        /// and succeed afterwards.
        /// </summary>
        private void SetupVerifyFailsOnceWithNamedPackError(string packFileName)
        {
            int verifyCount = 0;
            this.gitProcess.SetExpectedCommandResult(
                this.VerifyCommand,
                () =>
                {
                    verifyCount++;
                    return verifyCount == 1
                        ? new GitProcess.Result(string.Empty, $"failed to load pack entry for oid[0] = abc\nerror: packfile {packFileName} does not match index\n", GitProcess.Result.GenericFailureCode)
                        : new GitProcess.Result(string.Empty, string.Empty, GitProcess.Result.SuccessCode);
                });
        }

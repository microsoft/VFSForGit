...
{
    [TestFixture]
    public class PhysicalFileSystemExtensionsTests
    {
        [TestCase]
        ...
        public void TryWaitForDeleteLogsAtSpecifiedInterval()
        {
            ...
            MockTracer mockTracer = new MockTracer();
            int maxRetries = 10;
            PhysicalFileSystemExtensions.TryWaitForDelete(fileSystem, mockTracer, path, retryDelayMs: 0, maxRetries: maxRetries, retryLoggingThreshold: 3).ShouldBeFalse();
            fileSystem.DeleteFileCallCount.ShouldEqual(maxRetries + 1);

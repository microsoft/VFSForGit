#include "stdafx.h"
#include "GVFlt_DeleteFolderTest.h"
...
bool GVFlt_DeleteVirtualNonEmptyFolder_SetDisposition(const char* virtualRootPath)
{
    try
    {
...
bool GVFlt_DeleteVirtualNonEmptyFolder_DeleteOnClose(const char* virtualRootPath)
{
    try
    {
...
bool GVFlt_DeletePlaceholderNonEmptyFolder_SetDisposition(const char* virtualRootPath)
{
    try
    {
...
bool GVFlt_DeleteNonRootVirtualFolder_SetDisposition(const char* virtualRootPath)
{
    try
    {
        ...
        // NOTE: Deviate from GVFlt's DeleteNonRootVirtualFolder_SetDisposition here by deleting a file first
...
bool GVFlt_DeleteNonRootVirtualFolder_DeleteOnClose(const char* virtualRootPath)
{
    try
    {
        ...
        // NOTE: Deviate from GVFlt's DeleteNonRootVirtualFolder_DeleteOnClose here by deleting a file first
        // Git will not allow empty folders to be commited, and so \E must have a file in it
        DWORD fileError = DelFile(testScratchRoot + testFolder + targetFolder + testFile);
        VERIFY_ARE_EQUAL((DWORD)ERROR_SUCCESS, fileError);

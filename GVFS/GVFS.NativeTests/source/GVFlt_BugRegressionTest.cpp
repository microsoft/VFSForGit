#include "stdafx.h"
#include "GVFlt_BugRegressionTest.h"
...
bool GVFlt_ModifyFileInScratchAndDir(const char* virtualRootPath)
{
    // For bug #7700746 - File size is not updated when writing to a file projected from GVFlt app (e.g. GVFS, test app)
...
{
    // Bug #7703883 - GvFlt: RMDIR /s against a partial folder returns directory not empty error
...
bool GVFlt_RMDIRTest3(const char* virtualRootPath)
{
    try
    {
        // layer: 1, 3

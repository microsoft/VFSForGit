...
namespace GVFS.CommandLine
{
    public abstract class GVFSVerb
    {
        ...
        protected void CheckGVFltRunning()
        {
            ...
            {
                this.CheckFlightSigningEnabled();
        ...
        private void CheckFlightSigningEnabled()
        {
            ProcessResult result = ProcessHelper.Run("bcdedit", string.Empty);
            string[] ouptut = result.Output.Split('\n');
            foreach (string line in ouptut)
            {
                string upperCaseLine = line.ToUpperInvariant();
                if ((upperCaseLine.Contains("FLIGHTSIGNING") && upperCaseLine.Contains("YES")) ||
                     (upperCaseLine.Contains("TESTSIGNING") && upperCaseLine.Contains("YES")))
                {
                    return;
                }
            }

            string enableFlightSigningMessage =
@"
Flight signing must be enabled to use GVFS.  

To enable flight signing:
  1. Suspend BitLocker on OS Drive (Control Panel > System and Security > BitLocker Drive Encryption > Suspend Protection)
  2. Open CMD with elevated privileges and run 'bcdedit -set FLIGHTSIGNING ON'
  3. Restart machine
     ##################################################################
     # IMPORTANT: This step is essential, machine MUST be restarted!
     ##################################################################

";

            this.ReportErrorAndExit(enableFlightSigningMessage);
        }

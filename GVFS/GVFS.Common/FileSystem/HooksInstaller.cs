...
namespace GVFS.Common.FileSystem
{
    public static class HooksInstaller
    {
        ...
        public static bool InstallHooks(GVFSContext context, out string error)
        {
            ...
            try
            {
                foreach (HookData hook in NativeHooks)
                {
                    ...
                    string targetHookPath = Path.Combine(context.Enlistment.StorageRoot, hook.Path + GVFSPlatform.Instance.Constants.ExecutableExtension);
                ...
                string precommandHookPath = Path.Combine(context.Enlistment.StorageRoot, GVFSConstants.DotGit.Hooks.PreCommandPath);
                ...
                string postcommandHookPath = Path.Combine(context.Enlistment.StorageRoot, GVFSConstants.DotGit.Hooks.PostCommandPath);
        ...
        {
            bool copyHook = false;
            string enlistmentHookPath = Path.Combine(context.Enlistment.StorageRoot, hook.Path + GVFSPlatform.Instance.Constants.ExecutableExtension);
            string installedHookPath = Path.Combine(ExecutingDirectory, hook.ExecutableName);

...
namespace GVFS.Common.Maintenance
{
    ...
    public class PackfileMaintenanceStep : GitMaintenanceStep
    {
        ...
        private void RepairMultiPackIndex(ITracer activity, GitProcess.Result failure, bool recoveryEnabled)
        {
            ...
            {
                this.DetectAndRemoveCorruptPacks(activity, recoveryEnabled, out prefetchRestoreNeeded, failure.Errors);
            }
        ...
        /// </summary>
        // public only for unit tests
        public void DetectAndRemoveCorruptPacks(ITracer activity, bool recoveryEnabled, out bool prefetchRestoreNeeded, string failureErrors = null)
        {
            ...
            // Phase 1 - detection (read-only, always runs). git's own 'multi-pack-index verify' failure
            // text usually already names the specific unreadable pack (e.g. "packfile pack-1234.pack does
            // not match index"), so try verifying just those named packs first - much faster than
            // verify-pack'ing every pack in the object cache. Fall back to the full scan whenever no
            // candidate can be parsed, or when every candidate turns out to verify successfully (the
            // write/verify failure that got us here must then be explained by some other pack).
            HashSet<string> candidateIdxPaths = this.ExtractCandidateCorruptIdxPaths(failureErrors);
            List<DirectoryItemInfo> idxItemsToVerify = candidateIdxPaths.Count > 0
                ? packDirContents.Where(info => candidateIdxPaths.Contains(info.FullName)).ToList()
                : packDirContents;

            long? minCorruptPrefetchTimestamp;
            List<string> corruptNonPrefetchIdxPaths;
            HashSet<string> corruptIdxPaths = this.VerifyPacksAndReportCorruption(
                activity,
                recoveryEnabled,
                idxItemsToVerify,
                out corruptNonPrefetchIdxPaths,
            ...
            if (corruptIdxPaths.Count == 0 && idxItemsToVerify != packDirContents)
            {
                // The candidate(s) parsed from the failure text turned out to be healthy - fall back to
                // verifying every pack so a real corruption elsewhere is not missed.
                corruptIdxPaths = this.VerifyPacksAndReportCorruption(
                    activity,
                    recoveryEnabled,
                    packDirContents,
                    out corruptNonPrefetchIdxPaths,
        ...
        /// <summary>
        /// Matches pack/idx file names (e.g. "pack-&lt;hash&gt;.pack", "prefetch-123-abc.idx") as they
        /// appear embedded in git's own error text - see packfile.c's "packfile %s does not match
        /// index" / "packfile %s index unavailable" and "wrong index v2 file size in %s" messages. Pack
        /// file names only ever contain word characters, hyphens, and dots, so this is precise and won't
        /// pick up unrelated substrings.
        /// </summary>
        private static readonly Regex CorruptPackFileNamePattern = new Regex(@"[\w\-]+\.(?:pack|idx)", RegexOptions.Compiled);

        /// <summary>
        /// Parses candidate corrupt pack file names directly out of a 'multi-pack-index write/verify'
        /// failure's stderr, returning their .idx paths under <see cref="GVFSEnlistment.GitPackRoot"/>.
        /// Git's own error text usually already names the specific unreadable packfile, so this lets the
        /// caller skip a full verify-pack scan of every pack in the object cache. Only paths that
        /// actually exist on disk are returned, since the parsed text could (rarely) reference a pack
        /// from a different object-dir or a message format this pattern doesn't recognize (e.g. the
        /// ordinal-only "could not load pack N" from the write path, which names no file at all).
        /// </summary>
        private HashSet<string> ExtractCandidateCorruptIdxPaths(string failureErrors)
        {
            HashSet<string> idxPaths = new HashSet<string>(GVFSPlatform.Instance.Constants.PathComparer);
            if (string.IsNullOrEmpty(failureErrors))
            {
                return idxPaths;
            }

            string packRoot = this.Context.Enlistment.GitPackRoot;
            foreach (Match match in CorruptPackFileNamePattern.Matches(failureErrors))
            {
                string idxFileName = Path.GetFileNameWithoutExtension(match.Value) + ".idx";
                string idxPath = Path.Combine(packRoot, idxFileName);
        ...
        /// <summary>
        /// Runs 'git verify-pack' against each .idx in <paramref name="idxItemsToVerify"/> that has a
        /// matching .pack on disk, and reports (via telemetry) every one that fails to verify. verify-pack
        /// is an external git process, so it is safe to run with the LibGit2 repo open.
        /// </summary>
        private HashSet<string> VerifyPacksAndReportCorruption(
            ITracer activity,
            bool recoveryEnabled,
            List<DirectoryItemInfo> idxItemsToVerify,
            out List<string> corruptNonPrefetchIdxPaths,
            out long? minCorruptPrefetchTimestamp)
        {
            minCorruptPrefetchTimestamp = null;
            corruptNonPrefetchIdxPaths = new List<string>();
            HashSet<string> corruptIdxPaths = new HashSet<string>(GVFSPlatform.Instance.Constants.PathComparer);

            foreach (DirectoryItemInfo info in idxItemsToVerify)
            {
                if (this.Stopping)
                {
                    return corruptIdxPaths;
                }

...
namespace GVFS.Service
{
    public static class ProcessAsCurrentUser
    {
        ...
        public static bool Run(ITracer tracer, string processName, string args, bool showWindow)
        {
            IntPtr environment = IntPtr.Zero;
            IntPtr duplicate = IntPtr.Zero;
            IntPtr userToken = GetCurrentUserToken(tracer);
                    ...
                    out duplicate))
                {
                    if (CreateEnvironmentBlock(ref environment, duplicate, false))
                    {
                        ...
                        if (CreateProcessAsUser(
                            ...
                            creationFlags: (showWindow ? 0 : CreateProcessFlags.CREATE_NO_WINDOW) | CreateProcessFlags.CREATE_UNICODE_ENVIRONMENT,
                            ...
                            processInformation: out procInfo))
                        {
                            try
                            {
                                tracer.RelatedInfo("Started process '{0} {1}' with Id {2}", processName, args, procInfo.ProcessId);
                            }
                            finally
                            {
                        ...
                        {
                            TraceWin32Error(tracer, "Unable to start process.");
                        }
                    }
                    else
                    {
                        TraceWin32Error(tracer, "Unable to set child process environment block.");
                    }
                }
                else
                {
                    TraceWin32Error(tracer, "Unable to duplicate user token.");
                }
            }
            finally
            {
        ...
        private static IntPtr GetCurrentUserToken(ITracer tracer)
        {
            List<WTS_SESSION_INFO> sessions = ListSessions(tracer);
            if (sessions.Count != 0)
            {
                foreach (WTS_SESSION_INFO sessionInfo in sessions)
                {
                    if (sessionInfo.State == ConnectionState.Active)
                    {
                        ...
                        if (sessionId != int.MaxValue)
                        {
                            ...
                            {
                                TraceWin32Error(tracer, string.Format("Unable to query user token from active session '{0}'.", sessionId));
                            }
                        }
                    }
                }

                tracer.RelatedError("Could not obtain a valid user token as no active sessions were found on this host. Defaulting to the user token used to start the service.");

...
namespace GVFS.Platform.Linux
{
    public class LinuxFileSystemVirtualizer : FileSystemVirtualizer
    {
        ...
        public override FileSystemResult WritePlaceholderFile(
            ...
            string sha)
        {
            // TODO(Linux): Add functional tests that validate file mode is set correctly
            ...
            out UpdateFailureReason failureReason)
        {
            ...
            // TODO(Linux): Add functional tests that include:
            ...
            try
            {
                if (!this.GitObjects.TryCopyBlobContentStream(
                    ...
                    (stream, blobLength) =>
                    {
                        ...
                        // TODO(Linux): Find a better solution than reading from the stream one byte at at time
        ...
        private Result OnPreRename(string relativePath, string relativeDestinationPath, bool isDirectory)
        {
            ...
            {
                bool pathInsideDotGit = Virtualization.FileSystemCallbacks.IsPathInsideDotGit(relativePath);
                bool destinationPathInsideDotGit = Virtualization.FileSystemCallbacks.IsPathInsideDotGit(relativeDestinationPath);
                if ((pathInsideDotGit &&
                     relativePath.Equals(GVFSConstants.DotGit.Index, StringComparison.Ordinal)) ||
                    (destinationPathInsideDotGit &&

    ...
    public class WindowsFileSystemVirtualizer : FileSystemVirtualizer, IRequiredCallbacks
    {
        ...
        public HResult EndDirectoryEnumerationCallback(Guid enumerationId)
        {
            try
            {
                ...
                {
                    // This End removed nothing from the active set. If GVFS's own eviction removed the
                    // ID, keep the ended marker (Evicted wins classification regardless). Otherwise GVFS
                    // never actually held this ID, so undo the marker recorded above, so a later Get is
                    // classified NeverSeen rather than skewed to EndedRecently. The record-before-remove
                    // ordering still holds for the normal successful-remove path, so a Get racing a real
                    // End stays race-safe.
                    if (!this.enumerationFailureTracker.IsRecentlyEvicted(enumerationId))
                    {
                        this.enumerationFailureTracker.UndoEnded(enumerationId);
                    }

...
namespace RGFS.CommandLine.DiskLayoutUpgrades
{
    public abstract class DiskLayoutUpgrade
    {
        ...
        {
            using (JsonEtwTracer tracer = new JsonEtwTracer(RGFSConstants.RGFSEtwProviderName, "DiskLayoutUpgrade"))
            {
                try
                {
        ...
        public static bool TryCheckDiskLayoutVersion(ITracer tracer, string enlistmentRoot, out string error)
        {
            ...
            try
            {
                if (TryGetDiskLayoutVersion(tracer, enlistmentRoot, out persistedVersionNumber, out error))
                {
                    ...
                    {
                        error = string.Format(
                            "Breaking change to RGFS disk layout has been made since cloning. \r\nEnlistment disk layout version: {0} \r\nRGFS disk layout version: {1} \r\nMinimum supported version: {2}",
                    ...
                    {
                        error = string.Format(
                            "Changes to RGFS disk layout do not allow mounting after downgrade. Try mounting again using a more recent version of RGFS. \r\nEnlistment disk layout version: {0} \r\nRGFS disk layout version: {1}",
                            persistedVersionNumber,
                            RepoMetadata.DiskLayoutVersion.CurrentDiskLayoutVersion);
                    ...
                    {
                        error = string.Format(
                            "RGFS disk layout version doesn't match current version. Try running 'rgfs mount' to upgrade. \r\nEnlistment disk layout version: {0} \r\nRGFS disk layout version: {1}",
                            persistedVersionNumber,
                            RepoMetadata.DiskLayoutVersion.CurrentDiskLayoutVersion);
            ...
            error = "Failed to read disk layout version. " + ConsoleHelper.GetRGFSLogMessage(enlistmentRoot);
            return false;
        }
        ...
        protected bool TryIncrementDiskLayoutVersion(ITracer tracer, string enlistmentRoot, DiskLayoutUpgrade upgrade)
        {
            ...
            string dotRGFSPath = Path.Combine(enlistmentRoot, RGFSConstants.DotRGFS.Root);
            string error;
            if (!RepoMetadata.TryInitialize(tracer, dotRGFSPath, out error))
            {
        ...
        {
            string dotRGFSPath = Path.Combine(enlistmentRoot, RGFSConstants.DotRGFS.Root);
            string repoMetadataPath = Path.Combine(dotRGFSPath, EsentRepoMetadataName);
        ...
        private static void StartLogFile(string enlistmentRoot, JsonEtwTracer tracer)
        {
            if (!tracer.HasLogFileEventListener)
            {
                tracer.AddLogFileEventListener(
                    RGFSEnlistment.GetNewRGFSLogFileName(
                        Path.Combine(enlistmentRoot, RGFSConstants.DotRGFS.LogPath),
                        RGFSConstants.LogFileTypes.Upgrade),

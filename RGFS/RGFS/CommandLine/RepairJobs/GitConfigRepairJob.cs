...
namespace RGFS.CommandLine.RepairJobs
{
    ...
    {
        public GitConfigRepairJob(ITracer tracer, TextWriter output, RGFSEnlistment enlistment)
        ...
        public override IssueType HasIssue(List<string> messages)
        {
            ...
            if (result.HasErrors)
            {
                ...
                else if (result.Errors.Contains("--local"))
                {
                    ...
                    messages.Add("An issue was found that may be a side-effect of other issues. Fix them with 'rgfs repair --confirm' then 'rgfs repair' again.");
                    return IssueType.CantFix;
                }
            ...
            try
            {
                RGFSEnlistment enlistment = RGFSEnlistment.CreateFromDirectory(
                    this.Enlistment.EnlistmentRoot,
                    this.Enlistment.GitBinPath,
                    this.Enlistment.RGFSHooksRoot);
            ...
            {
                messages.Add("Authentication failed. Run 'rgfs log' for more info.");
        ...
        {
            string configPath = Path.Combine(this.Enlistment.WorkingDirectoryRoot, RGFSConstants.DotGit.Config);
            ...
            if (!RGFSVerb.TrySetGitConfigSettings(this.Enlistment))
            {

...
namespace RGFS.CommandLine.RepairJobs
{
    ...
    {
        public GitHeadRepairJob(ITracer tracer, TextWriter output, RGFSEnlistment enlistment) 
        ...
        public override FixResult TryFixIssues(List<string> messages)
        {
            string error;
            RefLogEntry refLog;
            if (!TryReadLastRefLogEntry(this.Enlistment, RGFSConstants.DotGit.HeadName, out refLog, out error))
            {
            ...
            try
            {
                string refPath = Path.Combine(this.Enlistment.WorkingDirectoryRoot, RGFSConstants.DotGit.Head);
        ...
        {
            string refLogPath = Path.Combine(enlistment.WorkingDirectoryRoot, RGFSConstants.DotGit.Logs.Root, fullSymbolicRef);
        ...
        {
            string refPath = Path.Combine(enlistment.WorkingDirectoryRoot, RGFSConstants.DotGit.Head);
            if (!File.Exists(refPath))
            {
                messages.Add("Could not find ref file for '" + RGFSConstants.DotGit.Head + "'");
                return false;
            }
            ...
            messages.Add("Invalid contents found in '" + RGFSConstants.DotGit.Head + "': " + refContents);
            return false;
        }
        ...
        private bool CanBeRepaired(List<string> messages)
        {
            ...
            string rebasePath = Path.Combine(this.Enlistment.WorkingDirectoryRoot, RGFSConstants.DotGit.RebaseApply);
            ...
            string mergeHeadPath = Path.Combine(this.Enlistment.WorkingDirectoryRoot, RGFSConstants.DotGit.MergeHead);
            ...
            string bisectStartPath = Path.Combine(this.Enlistment.WorkingDirectoryRoot, RGFSConstants.DotGit.BisectStart);
            ...
            string cherrypickHeadPath = Path.Combine(this.Enlistment.WorkingDirectoryRoot, RGFSConstants.DotGit.CherryPickHead);
            ...
            string revertHeadPath = Path.Combine(this.Enlistment.WorkingDirectoryRoot, RGFSConstants.DotGit.RevertHead);

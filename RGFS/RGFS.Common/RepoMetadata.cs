...
namespace RGFS.Common
{
    public class RepoMetadata
    {
        ...
        public static bool TryInitialize(ITracer tracer, string dotRGFSPath, out string error)
        {
            string dictionaryPath = Path.Combine(dotRGFSPath, RGFSConstants.DotRGFS.Databases.RepoMetadata);
            if (Instance != null)
            {
        ...
        public static class DiskLayoutVersion
        {
            ...
            // that would impact and older RGFS's ability to mount the repo
            public const int CurrentDiskLayoutVersion = 11;

            public const string MissingVersionError = "Enlistment disk layout version not found, check if a breaking change has been made to RGFS since cloning this enlistment.";

            // MaxDiskLayoutVersion ensures that olders versions of RGFS will not try to mount newer enlistments (if the 
            // disk layout of the newer RGFS is incompatible).
            // RGFS will only mount if the disk layout version of the repo is <= MaxDiskLayoutVersion
            public const int MaxDiskLayoutVersion = CurrentDiskLayoutVersion;

            // MinDiskLayoutVersion ensures that RGFS will not attempt to mount an older repo if there has been a breaking format
            // change since that enlistment was cloned.
            //     - RGFS will only mount if the disk layout version of the repo is >= MinDiskLayoutVersion

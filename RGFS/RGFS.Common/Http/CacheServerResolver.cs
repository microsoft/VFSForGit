...
namespace RGFS.Common.Http
{
    ...
    {
        private const string CacheServerConfigName = "rgfs.cache-server";
            ...
            out string error)
        {
            ...
            {
                cacheServer =
                    rgfsConfig.CacheServers.FirstOrDefault(cache => cache.GlobalDefault)
                    ?? this.CreateNone();
            }
            else
            {
                cacheServer = rgfsConfig.CacheServers.FirstOrDefault(cache =>
            ...
            RGFSConfig rgfsConfig)
        {
            ...
            return
                rgfsConfig.CacheServers.FirstOrDefault(cache => cache.Url.Equals(cacheServerUrl, StringComparison.OrdinalIgnoreCase))
        ...
        private static string GetDeprecatedCacheConfigSettingName(Enlistment enlistment)
        {
            ...
            return RGFSConstants.GitConfig.RGFSPrefix + sectionUrl + DeprecatedCacheEndpointGitConfigSuffix;
        }

...
namespace RGFS.Service.UI
{
    public class RGFSServiceUI
    {
        private const string ServiceAppId = "RGFS";

        private readonly ITracer tracer;
        
        public RGFSServiceUI(ITracer tracer)
        {
            this.tracer = tracer;
        }
        ...
        {
            using (JsonEtwTracer tracer = new JsonEtwTracer("Microsoft.Git.RGFS.Service.UI", "Service.UI"))
            {
                string logLocation = Path.Combine(
                    Environment.GetEnvironmentVariable("LocalAppData"),
                    RGFSConstants.Service.UIName,
                    "serviceUI.log");
                
                tracer.AddLogFileEventListener(logLocation, EventLevel.Informational, Keywords.Any);
                RGFSServiceUI process = new RGFSServiceUI(tracer);
                process.Start(args);
            }
        }
        ...
        private void Start(string[] args)
        {
            ...
            using (NamedPipeServer server = NamedPipeServer.StartNewServer(RGFSConstants.Service.UIName, this.tracer, this.HandleRequest))
            {
                ServiceController controller = new ServiceController(RGFSConstants.Service.ServiceName);
                try
                {
                ...
                this.tracer.RelatedInfo("{0} stop detected -- exiting UI.", RGFSConstants.Service.ServiceName);
            }
        }

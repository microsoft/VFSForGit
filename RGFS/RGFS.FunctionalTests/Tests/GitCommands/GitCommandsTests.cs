...
namespace RGFS.FunctionalTests.Tests.GitCommands
{
    ...
    public class GitCommandsTests : GitRepoTests
    {
        ...
        private const string EditFilePath = @"RGFS\RGFS.Common\RGFSContext.cs";
        private const string DeleteFilePath = @"RGFS\RGFS\Program.cs";
        private const string RenameFilePathFrom = @"RGFS\RGFS.Common\Physical\FileSystem\FileProperties.cs";
        private const string RenameFilePathTo = @"RGFS\RGFS.Common\Physical\FileSystem\FileProperties2.cs";
        private const string RenameFolderPathFrom = @"RGFS\RGFS.Common\PrefetchPacks";
        private const string RenameFolderPathTo = @"RGFS\RGFS.Common\PrefetchPacksRenamed";
        ...
        public void AddFileAndCommitOnNewBranchSwitchDeleteFolderAndSwitchBack()
        {
            ...
            string newFileParentFolderPath = @"RGFS\RGFS\CommandLine";
        ...
        public void MoveFileFromOutsideRepoToInsideRepoAndAdd()
        {
            ...
            string rgfsFilePath = Path.Combine(this.Enlistment.RepoRoot, filename);
            ...
            // Move file to RGFS repo
            this.FileSystem.WriteAllText(oldFilePath, testFileContents);
            this.FileSystem.MoveFile(oldFilePath, rgfsFilePath);
            oldFilePath.ShouldNotExistOnDisk(this.FileSystem);
            rgfsFilePath.ShouldBeAFile(this.FileSystem).WithContents(testFileContents);
        ...
        public void MoveFolderFromOutsideRepoToInsideRepoAndAdd()
        {
            ...
            string rgfsFolderPath = Path.Combine(this.Enlistment.RepoRoot, folderName);
            ...
            // Move folder to RGFS repo
            this.FileSystem.CreateDirectory(oldFolderPath);
            this.FileSystem.WriteAllText(oldFilePath, testFileContents);
            this.FileSystem.MoveDirectory(oldFolderPath, rgfsFolderPath);
            oldFolderPath.ShouldNotExistOnDisk(this.FileSystem);
            Path.Combine(rgfsFolderPath, filename).ShouldBeAFile(this.FileSystem).WithContents(testFileContents);
        ...
        public void MoveFileFromInsideRepoToOutsideRepoAndCommit()
        {
            ...
            string rgfsTargetFolder = "MoveFileFromInsideRepoToOutsideRepoAndCommit_RGFSTarget";
            ...
            string rgfsTargetFolderPath = Path.Combine(this.Enlistment.EnlistmentRoot, rgfsTargetFolder);
            string controlTargetFilePath = Path.Combine(controlTargetFolderPath, fileName);
            string rgfsTargetFilePath = Path.Combine(rgfsTargetFolderPath, fileName);
            ...
            // Move RGFS repo file
            this.FileSystem.CreateDirectory(rgfsTargetFolderPath);
            this.FileSystem.MoveFile(Path.Combine(this.Enlistment.RepoRoot, fileName), rgfsTargetFilePath);
            rgfsTargetFilePath.ShouldBeAFile(this.FileSystem);
        ...
        public void AddFileCommitThenDeleteAndCommit()
        {
            ...
            string filePath = @"RGFS\testfile.txt";
        ...
        public void ResetMixedTwiceThenCheckout()
        {
            ...
            // This test will fail if the checkout get the RGFS lock before the BG thread
            // because the always_exclude will not be updated and the RGFS repo will not warn
        ...
        public void OpenFileThenCheckout()
        {
            ...
            // will ensure that RGFS is still reporting the edited file as modified.

            string controlRepoRoot = this.ControlGitRepo.RootPath;
            string rgfsRepoRoot = this.Enlistment.RepoRoot;
            ...
            ProcessResult actualResult = GitHelpers.InvokeGitAgainstRGFSRepo(rgfsRepoRoot, command);
        ...
        {
            ProcessResult result = GitHelpers.InvokeGitAgainstRGFSRepo(this.Enlistment.RepoRoot, "update-index --skip-worktree Readme.md");
            result.Errors.ShouldContain("Modifying the skip worktree bit is not supported on a RGFS repo");

            result = GitHelpers.InvokeGitAgainstRGFSRepo(this.Enlistment.RepoRoot, "update-index --no-skip-worktree Readme.md");
            result.Errors.ShouldContain("Modifying the skip worktree bit is not supported on a RGFS repo");
        }

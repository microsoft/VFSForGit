...
namespace RGFS.FunctionalTests.Tests.GitCommands
{
    ...
    public abstract class GitRepoTests
    {
        ...
        protected RGFSFunctionalTestEnlistment Enlistment
        {
            get; private set;
        }
        ...
        protected void CreateEnlistment(string commitish = null)
        {
            ...
            this.Enlistment = RGFSFunctionalTestEnlistment.CloneAndMount(pathToRgfs, commitish: commitish);
         ...
         *    so the control repo will show the untracked files as being overwritten while the RGFS
         *    repo which is using the sparse-checkout will not.
         * 3. RGFS is returning not found for files that are outside the sparse-checkout and there
         *    are cases when git will delete these files during a merge outputting that it removed them
         *    which the RGFS repo did not have to remove so the message is missing that output.
         */

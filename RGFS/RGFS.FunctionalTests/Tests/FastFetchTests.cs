...
using RGFS.FunctionalTests.Properties;
...
namespace RGFS.FunctionalTests.Tests
{
    ...
    public class FastFetchTests
    {
        ...
        public void InitControlRepo()
        {
            ...
            GitProcess.Invoke("C:\\", "clone -b " + Settings.Default.Commitish + " " + RGFSTestConfig.RepoToClone + " " + this.fastFetchControlRoot);
        }
        ...
        public void InitRepo()
        {
            ...
            GitProcess.Invoke(this.fastFetchRepoRoot, "remote add origin " + RGFSTestConfig.RepoToClone);
        }
        ...
        {
            this.RunFastFetch("--checkout --folders \"/RGFS\" -b " + Settings.Default.Commitish);
            ...
            dirs.SequenceEqual(new[] 
            {
                ...
                Path.Combine(this.fastFetchRepoRoot, "RGFS"),
                Path.Combine(this.fastFetchRepoRoot, "RGFS.sln")
            });

            Directory.EnumerateFileSystemEntries(Path.Combine(this.fastFetchRepoRoot, "RGFS"), "*", SearchOption.AllDirectories)
                .Count()
                .ShouldEqual(345);

            this.AllFetchedFilePathsShouldPassCheck(path => path.StartsWith("RGFS", StringComparison.OrdinalIgnoreCase));
        }
        ...
        {
            // Initialize the repo
            GitProcess.Invoke(this.fastFetchRepoRoot, "config --local --add core.rgfs " + (indexSigningOff ? 1 : 0));
        ...
        public void IncrementalChangesLeaveGoodStatus()
        {
            ...
            GitProcess.Invoke(this.fastFetchRepoRoot, "config --local --add core.rgfs 1");

...
namespace FastFetch
{
    public class CheckoutFetchHelper : FetchHelper
    {
                ...
                using (ITracer activity = this.Tracer.StartActivity("UpdateIndex", EventLevel.Informational, Keywords.Telemetry, updateIndexMetadata))
                {
                    ...
                    if (!indexGen.HasFailures)
                    {
                        Index newIndex = new Index(
                            ...
                            this.Tracer,
                            Path.Combine(this.Enlistment.DotGitRoot, RGFSConstants.DotGit.IndexName), 
                            readOnly: false);
        ...
        {
            string indexPath = Path.Combine(this.Enlistment.DotGitRoot, RGFSConstants.DotGit.IndexName);
            string backupIndexPath = Path.Combine(this.Enlistment.DotGitRoot, RGFSConstants.DotGit.IndexName + ".backup");
        ...
        {
            // The first bit of core.rgfs is set if index signing is turned off.
            const uint CoreRgfsUnsignedIndexFlag = 1;

            GitProcess git = new GitProcess(this.Enlistment);
            GitProcess.Result configCoreRgfs = git.GetFromConfig("core.rgfs");
            uint valueCoreRgfs;

            // No errors getting the configuration and it is either "true" or numeric with the right bit set.
            return !configCoreRgfs.HasErrors &&
                !string.IsNullOrEmpty(configCoreRgfs.Output) &&
                (configCoreRgfs.Output.Equals("true", StringComparison.OrdinalIgnoreCase) ||
                (uint.TryParse(configCoreRgfs.Output, out valueCoreRgfs) &&
                ((valueCoreRgfs & CoreRgfsUnsignedIndexFlag) == CoreRgfsUnsignedIndexFlag)));
        }
    }
}

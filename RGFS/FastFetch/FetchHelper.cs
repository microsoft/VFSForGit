...
namespace FastFetch
{
    public class FetchHelper
    {
            ...
            int indexThreadCount)
        {
            ...
            // We never want to update config settings for a RGFSEnlistment
            this.SkipConfigUpdate = enlistment is RGFSEnlistment;
        }
            ...
            if (!string.IsNullOrWhiteSpace(folderListFile))
            {
                if (File.Exists(folderListFile))
                {
                    IEnumerable<string> allLines = File.ReadAllLines(folderListFile)
                        ...
                        .Where(line => !line.StartsWith(RGFSConstants.GitCommentSign.ToString()))
                        .Select(path => FetchHelper.ToAbsolutePath(enlistment, path, isFolder: true));
        ...
        public static bool TryLoadFileList(Enlistment enlistment, string filesInput, List<string> fileListOutput, out string error)
        {
            ...
            foreach (string file in fileListOutput)
            {
                ...
                if (file.EndsWith(RGFSConstants.GitPathSeparatorString) ||
                    file.EndsWith(RGFSConstants.PathSeparatorString))
                {
            ...
            out int readFileCount)
        {
            ...
            string shallowFile = Path.Combine(this.Enlistment.WorkingDirectoryRoot, RGFSConstants.DotGit.Shallow);
        ...
        protected virtual void UpdateRefs(string branchOrCommit, bool isBranch, GitRefs refs)
        {
            ...
            File.AppendAllText(Path.Combine(this.Enlistment.WorkingDirectoryRoot, RGFSConstants.DotGit.Shallow), commitSha + "\n");
        }
        ...
        private static string ToAbsolutePath(Enlistment enlistment, string path, bool isFolder)
        {
            string absolute = 
                path.StartsWith("*")
                ? path
                : Path.Combine(enlistment.WorkingDirectoryRoot, path.Replace(RGFSConstants.GitPathSeparator, RGFSConstants.PathSeparator).TrimStart(RGFSConstants.PathSeparator));

            if (isFolder &&
                !absolute.EndsWith(RGFSConstants.PathSeparatorString))
            {
                absolute += RGFSConstants.PathSeparatorString;
            }
